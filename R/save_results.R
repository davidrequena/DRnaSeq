...
#' @param name The name to be used to save the table.
...
save_results <- function(df, name, l2fc = 0){
  ...
  df.sig.fold_over <- subset(df, ((padj < cutoff_alpha) & !is.na(padj)) &
                               log2FoldChange >= l2fc)
  ...
  df.sig.fold_under <- subset(df, ((padj < cutoff_alpha) & !is.na(padj)) &
                                log2FoldChange <= -l2fc)
